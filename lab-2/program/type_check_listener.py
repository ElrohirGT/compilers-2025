...
class TypeCheckListener(SimpleLangListener):
    ...
    def exitMulDiv(self, ctx: SimpleLangParser.MulDivContext):
        ...
        if not self.is_valid_arithmetic_operation(left_type, right_type):
            self.errors.append(
                f"Unsupported operand types for * or /: {left_type} and {right_type}"
            )
        self.types[ctx] = (
            FloatType()
    ...
    def enterExpon(self, ctx: SimpleLangParser.ExponContext):
        pass

    def exitExpon(self, ctx: SimpleLangParser.ExponContext):
        left_type = self.types[ctx.expr(0)]
        right_type = self.types[ctx.expr(1)]
    ...
    def enterConcat(self, ctx: SimpleLangParser.ConcatContext):
        pass

    def exitConcat(self, ctx: SimpleLangParser.ConcatContext):
        left_type = self.types[ctx.expr(0)]
        right_type = self.types[ctx.expr(1)]

        if not isinstance(left_type, StringType) or not isinstance(
